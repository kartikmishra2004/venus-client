...
const UpdatePayment = ({ setShowModal, bookingId }: UpdatePaymentProps) => {
    ...
    const [booking, setBooking] = useState<PiPlayBooking | null>(null);
    ...
    const fetchBookingById = async () => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/piplay/${bookingId}`, {
                headers: {
                    // Add Authorization header if required here, e.g.:
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (additionalAmount <= 0 || additionalAmount > booking.pendingAmount) {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/piplay/payment/${bookingId}`, {
                method: 'PATCH',
                headers: {
    ...
    const handlePaymentChange = (value: string) => {
        ...
        if (booking && numValue >= 0 && numValue <= booking.pendingAmount) {
    ...
    const { totalAmount, advanceAmount, pendingAmount } = booking;
    const paymentPercentage = totalAmount > 0 ? (advanceAmount / totalAmount) * 100 : 0;
                    ...
                    <div className="bg-gray-50 dark:bg-zinc-800 rounded p-2">
                        <div className="text-xs space-y-1">
                            ...
                            <div className="flex justify-between">
                                ...
                                <span className="font-medium text-gray-900 dark:text-white">{booking.sportType}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-gray-600 dark:text-gray-400">Court No:</span>
                                <span className="font-medium text-gray-900 dark:text-white">{booking.courtNumber}</span>
                            </div>
                        </div>
                    </div>
